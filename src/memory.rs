pub trait Addressable {
    ...
    fn write2(&mut self, addr: u16, value: u16) -> bool {
        let lower: u16 = value & 0xff;
        let upper: u16 = (value & 0xff00) >> 8;
    ...
    fn from_vector(&mut self, from: Vec<u8>, addr: u16) -> Result<usize, String> {
        for (i, b) in from.iter().enumerate() {
            if !self.write(addr+(i as u16), *b) {
                return Err("couldn't write the commands from file".to_string());
            }
        }
        return Ok(from.len());
    }
